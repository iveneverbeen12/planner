...
const BUILTIN_TYPES = {
    work: { label: 'Работа', color: 'bg-sky-600', textColor: 'text-sky-700', dot: 'bg-sky-600', bgLight: 'bg-sky-50', icon: 'fa-briefcase' },
    family: { label: 'Семья', color: 'bg-rose-500', textColor: 'text-rose-700', dot: 'bg-rose-500', bgLight: 'bg-rose-50', icon: 'fa-users' },
    sport: { label: 'Спорт', color: 'bg-emerald-600', textColor: 'text-emerald-700', dot: 'bg-emerald-600', bgLight: 'bg-emerald-50', icon: 'fa-person-running' }
};
...
function loadCustomTypes() {
    ...
    if (!Array.isArray(customTypes)) customTypes = [];
    customTypes = customTypes.filter(function (x) {
...
function getCustomTypeById(id) {
    for (let i = 0; i < customTypes.length; i++) {
        if (customTypes[i].id === id) return customTypes[i];
    }
    return null;
}
...
function createCustomTypeFromPanel() {
    ...
    customTypes.push({ id: id, name: name, colorHex: colorHex });
    saveCustomTypes();
    hideNewCustomPanel();
...
function updateCustomTypeField(id, patch) {
    const idx = customTypes.findIndex(function (c) {
        return c.id === id;
    });
    if (idx === -1) return;
    if (patch.name !== undefined) customTypes[idx].name = patch.name;
    if (patch.colorHex !== undefined) customTypes[idx].colorHex = patch.colorHex;
    saveCustomTypes();
    renderTypesLegend();
    refreshActiveView();
...
function deleteCustomType(id) {
    ...
    customTypes = customTypes.filter(function (c) {
        return c.id !== id;
    });
    saveCustomTypes();
    events.forEach(function (e) {
...
function renderAllEventsFilterButtons() {
    ...
    mkBtn('all', 'Все', currentFilter === 'all');
    Object.keys(BUILTIN_TYPES).forEach(function (k) {
        mkBtn(k, BUILTIN_TYPES[k].label, currentFilter === k);
    });
    customTypes.forEach(function (ct) {
        mkBtn(ct.id, ct.name, currentFilter === ct.id);
    });
}
...
function loadPlannerSettings() {
    try {
        ...
        if (raw) plannerSettings = JSON.parse(raw);
    } catch (e) {}
    if (typeof plannerSettings.notificationsEnabled !== 'boolean') plannerSettings.notificationsEnabled = true;
    if (typeof plannerSettings.reminderCheckIntervalSec !== 'number' || isNaN(plannerSettings.reminderCheckIntervalSec)) {
        plannerSettings.reminderCheckIntervalSec = 30;
    }
    plannerSettings.reminderCheckIntervalSec = Math.min(600, Math.max(10, plannerSettings.reminderCheckIntervalSec));
    var allowedDr = ['', '0', '5', '15', '30', '60', '120', '1440'];
    if (plannerSettings.defaultReminderMinutes === undefined || plannerSettings.defaultReminderMinutes === null) {
        plannerSettings.defaultReminderMinutes = '';
    }
    var drs = String(plannerSettings.defaultReminderMinutes);
    if (allowedDr.indexOf(drs) === -1) plannerSettings.defaultReminderMinutes = '';
    if (plannerSettings.colorTheme !== 'light' && plannerSettings.colorTheme !== 'dark' && plannerSettings.colorTheme !== 'system') {
        plannerSettings.colorTheme = 'system';
    }
}
...
function getColorThemeMode() {
    var m = plannerSettings.colorTheme;
...
function setPlannerColorTheme(mode) {
    ...
    plannerSettings.colorTheme = mode;
    savePlannerSettings();
    applyColorTheme();
...
function scheduleReminderChecks() {
    ...
    var sec = Number(plannerSettings.reminderCheckIntervalSec);
    if (isNaN(sec)) sec = 30;
...
function setPlannerReminderIntervalSec(secVal) {
    ...
    if (isNaN(n)) n = 30;
    plannerSettings.reminderCheckIntervalSec = Math.min(600, Math.max(10, n));
    savePlannerSettings();
...
function setPlannerDefaultReminderMinutes(val) {
    ...
    plannerSettings.defaultReminderMinutes = allowed.indexOf(str) >= 0 ? str : '';
    savePlannerSettings();
    updateSettingsUI();
}
...
function checkReminders() {
    if (!plannerSettings.notificationsEnabled) return;
    const now = Date.now();
...
/** Последний день (воскресенье) недели, где лежит dateStr (неделя с понедельника). YYYY-MM-DD */
...
function renderDayView() {
    ...
    if (lbl) {
        const dd = new Date(currentDate.getFullYear(), currentDate.getMonth(), currentDate.getDate());
...
function renderUpcoming() {
    ...
    for (let i = 0; i < 8; i++) {
        ...
        const ds = ymd(d);
        events.forEach(function (ev) {
            if (!eventOccursOnDate(ev, ds)) return;
    ...
    top.forEach(function (x) {
        ...
        row.onclick = function () {
            ...
            setPlannerView('day');
            currentDate = new Date(x.dateStr + 'T12:00:00');
            renderDayView();
        };
        el.appendChild(row);
    });
...
function renderCalendar() {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        const dayEvents = events.filter(function (e) {
            return eventOccursOnDate(e, dateStr);
        });
    ...
    document.getElementById('events-count').innerHTML = events.length + ' событий';
    renderUpcoming();
}
...
function selectDate(dateStr) {
    ...
    document.getElementById('selected-date-header').innerHTML = formatted.split(',')[0];
    document.getElementById('selected-date-sub').innerHTML = dateObj.toLocaleDateString('ru-RU', { year: 'numeric' });
    if (plannerView === 'month') renderCalendar();
    else if (plannerView === 'week') renderWeekView();
    else renderDayView();
...
function showAddModal(preselectedDate) {
    ...
    var _drm = plannerSettings.defaultReminderMinutes;
...
function showAddModalForSelected() {
    if (!selectedDate) showAddModal(ymd(new Date()));
    else showAddModal(selectedDate);
}
...
function saveEvent(e) {
    ...
    const eventData = {
        ...
        attachments: JSON.parse(JSON.stringify(eventAttachmentsDraft))
    };

    if (editingEventId) {
        const index = events.findIndex(function (ev) {
    ...
    refreshActiveView();
    if (selectedDate) renderDayEvents(selectedDate);
    showToast(editingEventId ? 'Обновлено' : 'Событие добавлено');
}
...
function showEditModal(eventId) {
    ...
    eventAttachmentsDraft = JSON.parse(JSON.stringify(event.attachments || []));
    renderAttachmentDraftList();
    document.getElementById('event-modal').classList.remove('hidden');
    document.getElementById('event-modal').classList.add('flex');
}
...
function deleteEvent(eventId, fromSidebar) {
    var evDel = events.find(function (e) {
        return e.id === eventId;
    });
    var delMsg =
    ...
    saveEvents();
    refreshActiveView();
    if (fromSidebar && selectedDate) renderDayEvents(selectedDate);
    const allModal = document.getElementById('all-events-modal');
    if (allModal && !allModal.classList.contains('hidden')) renderAllEventsList();
    showToast('Удалено');
}
...
function renderAllEventsList() {
    ...
    container.innerHTML = '';
    let filteredEvents =
        currentFilter === 'all' ? events.slice() : events.filter(function (e) {
              return normalizeEventType(e.type) === currentFilter;
          });
    filteredEvents.sort(function (a, b) {
        return a.date.localeCompare(b.date) || (a.time || '').localeCompare(b.time || '');
    });
    document.getElementById('all-events-count').innerHTML = filteredEvents.length + ' событий';
...
function deleteFromAllEvents(eventId) {
    ...
    if (selectedDate) {
        if (events.some(function (e) {
            return eventOccursOnDate(e, selectedDate);
        }))
...
function exportEvents() {
    const payload = { version: 2, events: events, customTypes: customTypes };
...
function importEvents(ev) {
    ...
    reader.onload = function (e) {
        try {
            ...
            if (incomingCustom) {
                customTypes = incomingCustom.filter(function (x) {
                    return x && typeof x.id === 'string' && x.id.indexOf('custom_') === 0 && typeof x.name === 'string' && typeof x.colorHex === 'string';
                });
                saveCustomTypes();
                renderTypesLegend();
...
function togglePlannerNotifySetting() {
    plannerSettings.notificationsEnabled = !plannerSettings.notificationsEnabled;
    savePlannerSettings();
    updateSettingsUI();
}
...
function updateSettingsUI() {
    ...
    if (toggle) {
        const on = plannerSettings.notificationsEnabled;
    ...
    if (intervalSel) {
        var iv = String(plannerSettings.reminderCheckIntervalSec);
    ...
    if (defSel) {
        var dv = plannerSettings.defaultReminderMinutes === undefined || plannerSettings.defaultReminderMinutes === null
            ? ''
            : String(plannerSettings.defaultReminderMinutes);
        defSel.value = dv;
...
function initKeyboardShortcuts() {
    document.addEventListener('keydown', function (e) {
        if (e.key === '/' && document.activeElement.tagName === 'BODY') {
            e.preventDefault();
            const allModal = document.getElementById('all-events-modal');
            if (!allModal.classList.contains('hidden')) closeAllEventsModal();
        ...
        if (e.key === 'Escape') {
            ...
            else if (!allModal.classList.contains('hidden')) closeAllEventsModal();
...
function removeAttachmentDraft(attId) {
    eventAttachmentsDraft = eventAttachmentsDraft.filter(function (a) {
        return a.id !== attId;
    });
    renderAttachmentDraftList();
}
...
function renderAttachmentDraftList() {
    ...
    if (!host) return;
    if (eventAttachmentsDraft.length === 0) {
        host.innerHTML = '';
        return;
    }
    host.innerHTML = '';
    eventAttachmentsDraft.forEach(function (att) {
...
function handleEventFilesChange(inputEl) {
    ...
    Array.from(files).forEach(function (file) {
        ...
        reader.onload = function () {
            ...
            eventAttachmentsDraft.push({
                id: 'att-' + Date.now() + '-' + Math.random().toString(36).slice(2, 9),
                name: file.name,
                mime: file.type || 'application/octet-stream',
                data: base64
            });
            var total = JSON.stringify(events).length + JSON.stringify(eventAttachmentsDraft).length;
            if (total > 4500000) {
                eventAttachmentsDraft.pop();
